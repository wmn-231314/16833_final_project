...
RasterizeGaussiansCUDA(
    ...
    const torch::Tensor& projmatrix_raw, // new on monoGS
	...
	const bool debug)
{
  ...
  // compute n_touched out_depth out_opacity
  torch::Tensor n_touched = torch::full({P}, 0, means3D.options().dtype(torch::kInt32));
  torch::Tensor out_depth = torch::full({1, H, W}, 0.0, float_opts); // similar with color
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		out_depth.contiguous().data<float>(), // new on monoGS
		out_opaticy.contiguous().data<float>(), // new on monoGS
		radii.contiguous().data<int>(),
		n_touched.contiguous().data<int>(), // new on monoGS
        debug);
  }
