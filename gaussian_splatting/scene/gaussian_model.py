...
class GaussianModel:
    ...
    def capture_dict(self):
        return {
            ...
            "unique_kfIDs": self.unique_kfIDs.clone().detach().cpu(), # keyframe IDs for each point
            "n_obs": self.n_obs.clone().detach().cpu(),
            
            "optimizer": self.optimizer.state_dict(),
        }
        
    def reload_from_dict(self, ckpt_dict, opt_params):
        self.training_setup(opt_params)
        self.active_sh_degree = ckpt_dict["active_sh_degree"]
        self.densification_postfix(
            ckpt_dict["xyz"],
            ckpt_dict["features_dc"],
            ckpt_dict["features_rest"],
            ckpt_dict["opacity"],
            ckpt_dict["scaling"],
            ckpt_dict["rotation"],
            new_kf_ids=ckpt_dict["unique_kfIDs"],
            new_n_obs=ckpt_dict["n_obs"],
        )
